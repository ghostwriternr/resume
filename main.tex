...
\textsc{Aug 18} & {- Took ownership of the QBO billing platform's most used API and added clean abstraction, error and performance monitoring along with some enhancements for new business requirements.}\\
& {- Wrote Redis caching support for mobile offers during QBSE's migration to the team's new billing platform.}\\
& {- Identified \& filled monitoring gaps in billing platform's highly critical signup API with an extensible framework written in Java to publish metrics to Wavefront and analyze real-time data with region \& offering-wise alerts.}\\
...
& {- Extend support to tests as well, allowing incremental local and Google-internal CI (Kokoro) build \& test runs.}\\
